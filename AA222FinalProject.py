...
# Load the CSV files
cards_df = pd.read_csv('CreditCardDatabase.csv')
...
num_existing_cards = len(existing_card_ids) # Number of existing cards

# Indicator for pre-existing credit cards
...
def optimize(num_selected_cards, point_values):
    ...
    # Auxiliary variable
    Spending_Requirement_Met = model.addVars(num_cards, vtype=GRB.BINARY, name="Spending_Requirement_Met") # Whether Spending Requirement is met for eachc card

    # Primary Objective function: Net Positive Value Gained
    objective = quicksum(x[i] * point_values[i] * quicksum(earn_rates[i][j] * S[i, j] for j in range(len(categories))) - x[i] * annual_fees[i] + Spending_Requirement_Met[i] * point_values[i] * sign_up_bonus_points[i] * (1 - existing_card_indicator[card_ids[i]]) for i in range(num_cards))
    model.setObjective(objective, GRB.MAXIMIZE)

    # Constraints
    # Spend allocated to card i for spend category j cannot be negative
    for i in range(num_cards):
        for j in range(len(categories)):
            model.addConstr(S[i, j] >= 0, name=f"SpendNonNegative_{i}_{j}")
    # The total spend on spend category j across all cards must match the user-specified spend allocation
    for j in range(len(categories)):
        model.addConstr(quicksum(S[i, j] for i in range(num_cards)) == spend_amounts[j], name=f"TotalSpend_{j}")
    # Ensure no spend is allocated to a card that is not chosen
    for i in range(num_cards):
        for j in range(len(categories)):
            model.addConstr(S[i, j] <= spend_amounts[j] * x[i], name=f"SpendIfChosen_{i}_{j}")
    # Number of selected cards (For generating Pareto Frontier)
    model.addConstr(quicksum(x[i] for i in range(num_cards)) == num_selected_cards, name="NumSelectedCards")
    # Ensure the Spending_Requirement_Met variable reflects whether the spending requirement is met
    for i in range(num_cards):
    ...
    # Ensure cards in the existing credit card setup are selected
    for i in range(num_cards):
...
# Generate results for each value of num_selected_cards for the ThePointsGuy point values
results_original = []
pareto_frontier_original = []
for num_selected_cards in range(max(1, num_existing_cards), num_cards + 1):
    obj_val, selected_cards, model = optimize(num_selected_cards, point_values)
...
# Generate results for each value of num_selected_cards for the cash value of points
results_cash_value = []
pareto_frontier_cash_value = []
for num_selected_cards in range(max(1, num_existing_cards), num_cards + 1):
    obj_val, selected_cards, model = optimize(num_selected_cards, point_values_CashValue)
    if obj_val is not None:
        for selected_card in selected_cards:
            card_index = card_ids.index(selected_card)
            spending_breakdown = [model.getVarByName(f"S[{card_index},{j}]").x for j in range(len(categories))]
...
plt.plot(selected_cards_original, obj_values_original, marker='x', linestyle='None', label='Design Points for Pre-existing Credit Card Setup 1 (ThePointsGuy Valuation)', color='red')
...
plt.plot(pareto_cards_original, pareto_obj_values_original, marker='None', linestyle='-', linewidth=10, alpha=0.2, label='Pareto Frontier for Pre-existing Credit Card Setup 1 (ThePointsGuy Valuation)', color='red')
...
utopia_point_original = (max(1, num_existing_cards), max(obj_values_original))
plt.scatter(*utopia_point_original, color='magenta', zorder=5, label='Utopia Point for Pre-existing Credit Card Setup 1 (ThePointsGuy Valuation)')
utopia_point_cash_value = (max(1, num_existing_cards), max(obj_values_cash_value))
